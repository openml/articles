...
Listing datasets
```{r}
datasets = listOMLDataSets()  # returns all active data sets
datasets[1:20, c("data.id", "name", "number.of.instances", 
                 "number.of.features")]
```
...
---
Idem:
- List tasks (can take a few minutes)
```{r, eval=FALSE}
tasks = listOMLTasks()
```
...
```{r}
listOMLTasks(data.name = "GesturePhaseSegmentationRAW")[0:8]
```
...
evals[1:20, c("flow.name", "predictive.accuracy")]
```
...
$\rightarrow$`r paste0("https://www.openml.org/r/", run.id)`.

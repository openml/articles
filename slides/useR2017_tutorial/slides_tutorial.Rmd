...
- The server assigns a `run.id` which can be used to
  ...
  - download the run, `r sprintf("getOMLRun(%s)", run.id)` or
...
- Upload your run to OpenML. Add the tag "useR17" (Hint: `r run.id = uploadOMLRun(run.mlr, tag = "useR17")`). 
...
- Check the predictive performance of the run (It may take a while befor the server has calculated the performance measures).

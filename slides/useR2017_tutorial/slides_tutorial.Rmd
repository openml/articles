...
knitr::opts_chunk$set(dpi = 300, cache = TRUE, message = FALSE)
```
...
head(evals[, c("data.name", "learner.name", "predictive.accuracy")])
```
...
---
class:inverse
Why are the two different versions of the flow?
```{r}
